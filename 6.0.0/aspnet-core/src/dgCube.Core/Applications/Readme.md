...
- 前端代码生成的文件文件路径请在dgCube.Application类库中生成的对应实体代码文件中的Client中。
...
如果你选择了**生成权限功能**，请打开dgCube.Application类库中的dgCubeApplicationModule.cs类文件。
然后复制以下代码到 的PreInitialize 方法中:

```csharp
Configuration.Authorization.Providers.Add<ApplicationInfoAuthorizationProvider>();
...
请打开dgCube.Application类库中dgCubeApplicationModule.cs中的 PreInitialize 方法中:
...
打开EntityFrameworkCore类库在 **dgCubeDbContext**类文件中添加以下代码段：
...
{
   modelBuilder.ApplyConfiguration(new ApplicationInfoCfg());
 }
...
```
Add-Migration AddNewApplicationInfoEntity_Migration
```
...
生成的多语言内容
在dgCube.Core类库中对应实体下的`douyuyan.md`文件中。
